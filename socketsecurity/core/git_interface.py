...
class Git:
    ...
    def __init__(self, path: str):  # noqa: C901
        ...
        log.info(f"Git initialization completed in {time.perf_counter() - initialization_start:.2f}s")
    ...
    def _resolve_ref(self, ref: str | None) -> str | None:
        ...
        for candidate in candidates:
            try:
                ...
                log.debug(f"Could not resolve ref candidate {candidate}: {e}")
                continue
        return None
    ...
    def _detect_pull_request_changes(
        ...
        try:
            ...
            self.show_files = diff_files.splitlines()
            log.debug(f"Changed files detected via local git diff ({provider}): {self.show_files}")
            return True
        ...
        try:
            ...
            self.show_files = diff_files.splitlines()
            log.debug(f"Changed files detected after targeted fetch ({provider}): {self.show_files}")
            return True
        ...
        try:
            ...
            if buildkite_branch:
                if self._is_buildkite_pull_request(buildkite_pr):
                    log.debug(f"Processing Buildkite pull request from branch: {buildkite_branch}, not default branch")
                    return False
                ...
                log.debug(
                    f"Buildkite branch: {buildkite_branch}, Default: {default_branch_name}, Is default: {is_default}"
                )
                return is_default
            ...
            if github_ref:
                ...
                if github_ref.startswith("refs/heads/"):
                    ...
                    log.debug(
                        f"Branch from GITHUB_REF: {branch_from_ref}, Default: {default_branch_name}, Is default: {is_default}"
                    )
                    return is_default
            ...
            if bitbucket_branch:
                ...
                log.debug(
                    f"Bitbucket branch: {bitbucket_branch}, Default: {default_branch_name}, Is default: {is_default}"
                )
                return is_default
    ...
    def _detect_merge_commit_changes(self) -> bool:
        ...
        try:
            ...
            log.info(
                f"Changed file detection: method=merge-diff, source=merge-commit-fallback, files={len(self.show_files)}"
            )
            return True
        ...
        try:
            ...
            for branch_name in ["main", "master"]:
                try:
                    ...
                    log.debug(f"Could not check fallback branch {branch_name}: {e}")
                    continue
    ...
    def is_on_default_branch(self) -> bool:
        ...
        try:
            ...
            log.debug(
                f"Current branch: {current_branch_name}, Default branch: {default_branch_name}, Is default: {is_default}"
            )
            return is_default
    ...
    def ensure_safe_directory(path: str) -> None:
        ...
        try:
            ...
            if abs_path not in safe_dirs:
                subprocess.run(["git", "config", "--global", "--add", "safe.directory", abs_path], check=True)
