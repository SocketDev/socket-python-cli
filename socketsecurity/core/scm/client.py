...
class ScmClient(CliClient):
    ...
    def request(self, path: str, **kwargs):
        ...
        return super().request(path=path, headers=headers, base_url=self.api_url, **kwargs)


class GithubClient(ScmClient):
    def get_headers(self) -> dict:
        return {"Authorization": f"Bearer {self.token}", "User-Agent": USER_AGENT, "accept": "application/json"}
