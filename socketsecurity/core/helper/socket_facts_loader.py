...
def load_socket_facts(file_path: str = ".socket.facts.json") -> dict[str, Any] | None:
    """
...
def get_components_with_vulnerabilities(facts_data: dict[str, Any]) -> list:
    ...
    return [comp for comp in components if comp.get("vulnerabilities") and len(comp.get("vulnerabilities", [])) > 0]


def get_scan_id(facts_data: dict[str, Any]) -> str | None:
    """
...
def _determine_reachability(vulnerability: dict[str, Any], component: dict[str, Any]) -> dict[str, Any]:  # noqa: C901
    """
    ...
    """
    result = {"type": "unknown", "undeterminableReachability": False, "trace": []}
                    ...
                    if filename:
                        ...
                        if line is not None:
                            if end_line is not None and end_line != line:
                                loc = f"{filename} {line}:{col or ''}-{end_line}:{end_col or ''}"
                            else:
                                loc = f"{filename} {line}:{col or ''}"
                        result["trace"].append(loc)
                # Subsequent frames - show package/module reference
                elif pkg or filename:
                    entry = pkg or filename
                    entry = f"  -> {entry} {line}:{col or ''}" if line is not None else f"  -> {entry}"
...
def convert_to_alerts(components: list[dict[str, Any]]) -> list[dict[str, Any]]:  # noqa: C901
    """
