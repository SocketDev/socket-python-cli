...
class Helper:
    ...
    def parse_gfm_section(html_content):
        ...
        return {"before_html": before_html, "columns": headers, "rows_html": rows_html, "after_html": after_html}
    ...
    def parse_cell(html_td):
        ...
        if a:
            ...
            if img:
                cell.update(
                    {"img_src": img.get("src", ""), "title": img.get("title", ""), "link_text": a.get_text(strip=True)}
                )
            else:
        ...
        def handle_element(elem):
            if isinstance(elem, NavigableString):
                ...
                if elem.name == "a":
                    ...
                    parts.append(
                        {"img_src": elem.get("src", ""), "alt": elem.get("alt", ""), "title": elem.get("title", "")}
                    )
                else:
    ...
    def section_to_json(section_result):
        ...
        for row_html in section_result.get("rows_html", []):
            ...
            table_rows.append(dict(zip(cols, cells, strict=False)))
